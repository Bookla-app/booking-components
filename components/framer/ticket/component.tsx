...
const formatTicketsForApi = (selections: TicketSelection[]) => {
...
interface TicketPickerProps {
    ...
    fontFamily: string
    tickets: Ticket[]
    onTicketSelectionChange?: (selections: TicketSelection[]) => void
    initialSelections?: { [key: string]: number }
    onBack?: () => void
}

const TicketPicker: React.FC<TicketPickerProps> = ({
                                                       title,
                                                       subtitle,
                                                       size,
                                                       backgroundColor,
    ...
    const handleTicketSelection = (ticketId: string, checked: boolean) => {
        ...
        if (checked) {
            newSelectedTickets[ticketId] = 1
        } else {
            delete newSelectedTickets[ticketId]
        }
        setSelectedTickets(newSelectedTickets)
        notifySelectionChange(newSelectedTickets)
    }

    const handleCountChange = (ticketId: string, increment: boolean) => {
        const newSelectedTickets = { ...selectedTickets }
        const currentCount = newSelectedTickets[ticketId] || 0
        const newCount = increment
            ? currentCount + 1
            : Math.max(1, currentCount - 1)
        newSelectedTickets[ticketId] = newCount
        setSelectedTickets(newSelectedTickets)
        notifySelectionChange(newSelectedTickets)
    }

    const notifySelectionChange = (selections: { [key: string]: number }) => {
        if (onTicketSelectionChange) {
            ...
            onTicketSelectionChange(selectionArray)
        }
    }
    ...
    const checkedStyle = {
        ...checkboxStyle,
        backgroundColor: itemSelectColor,
        borderColor: itemSelectColor,
    }
    ...
    return (
        <div style={containerStyle}>
            ...
            <div style={ticketListStyle}>
                {tickets.map((ticket) => {
                    const isSelected = ticket.id in selectedTickets
                    return (
                        <div key={ticket.id} style={ticketItemStyle}>
                            <div
                                style={
                                    isSelected ? checkedStyle : checkboxStyle
                                }
                                onClick={() =>
                            ...
                            <div style={textContainerStyle}>
                                <div style={ticketNameStyle}>{ticket.name}</div>
                            </div>
                            {isSelected && (
                                <div style={counterContainerStyle}>
                                    <button
                                        ...
                                        onClick={() =>
                                            handleCountChange(ticket.id, false)
                                        }
                                    ...
                                    <button
                                        style={counterButtonStyle}
                                        onClick={() =>
                                            handleCountChange(ticket.id, true)
                                        }
...
// DatePicker Component (reused from other components)
...
const TicketSummary: React.FC<TicketSummaryProps> = ({
    ...
    return (
        <div style={containerStyle}>
            <div style={headerStyle}>
                <div style={titleStyle}>{summaryTitle}</div>
                <button style={manageButtonStyle} onClick={onManageClick}>
                    {manageText}
                </button>
            </div>
            <div style={ticketListStyle}>
...
// TimePicker Component (reused from other components)
...
// FormView Component (reused from other components)
...
// ResourcePicker Component (reused from other components)
interface Resource {
    id: string
    name: string
}
...
// AcceptTerms Component (reused from other components)
...
function TicketBookingComponent(props: TicketBookingProps) {
    ...
    useEffect(() => {
        if (serviceLoaded && selectedTickets.length > 0) {
            fetchAvailableDates()
        }
    }, [selectedResourceID, selectedTickets, serviceLoaded])
    ...
    const loadServiceData = async () => {
        ...
        try {
            ...
            // Check if service type is ticket
            if (response.type && response.type !== "ticket") {
                setError(
                    `This component only supports ticket services. Service type '${response.type}' is not supported.`
                )
                setServiceLoaded(false)
                return
            }
            ...
            const sortedTickets = (response.tickets || []).sort(
                (a: Ticket, b: Ticket) => a.name.localeCompare(b.name)
            )
    ...
    const fetchAvailableDates = async () => {
        ...
        try {
            ...
            const params: any = {
                ...
                tickets: formatTicketsForApi(selectedTickets),
            }
    ...
    const makeBooking = async () => {
        if (selectedTime === null || selectedTickets.length === 0) {
            return
        }
    ...
    const proceedToNextStep = () => {
        if (currentStep === BookingStep.TicketSelect) {
    ...
    const proceedToPreviousStep = () => {
        if (currentStep === BookingStep.Date) {
            setCurrentStep(BookingStep.TicketSelect)
            return
        }
        if (currentStep === BookingStep.Form) {
            setCurrentStep(BookingStep.Date)
    ...
    const bookButtonEnabled =
        ...
        (currentStep === BookingStep.TicketSelect ||
            currentStep === BookingStep.Form ||
            (currentStep === BookingStep.Date && selectedTime !== null)) &&
        (!props.showTerms ||
            termsAccepted ||
            currentStep !== BookingStep.Date) &&
        (currentStep !== BookingStep.TicketSelect || selectedTickets.length > 0)
    ...
    return (
        <div style={containerStyle}>
            ...
            {currentStep === BookingStep.TicketSelect && (
                <TicketPicker
                    title={props.texts.selectTicketsTitle}
                    subtitle={props.texts.selectTicketsSubtitle}
                    size={props.size}
            ...
            {currentStep === BookingStep.Date && (
                <>
                    ...
                    <TicketSummary
                        ...
                        border={props.blocks.border}
                        fontFamily={props.fontFamily}
                        summaryTitle={props.texts.summaryTitle}
                        manageText={props.texts.manageText}
...
TicketBookingComponent.propertyControls = {
    size: {
    ...
    texts: {
        ...
        controls: {
            ...
            selectTicketsSubtitle: {
                type: ControlType.String,
                title: "Select Tickets Subtitle",
                defaultValue: "Please select the tickets you need",
            },
            summaryTitle: {
                type: ControlType.String,
                title: "Summary Title",
                defaultValue: "Selected Tickets",
            },
            manageText: {
                type: ControlType.String,
                title: "Manage Text",
